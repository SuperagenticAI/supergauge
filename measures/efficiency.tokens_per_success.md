...
Token consumption divided by the number of tasks completed.

## Why it exists

Tokens are the currency-independent form of the same question, and the one that
stays comparable when provider pricing changes underneath a record.
...
```
efficiency.tokens_per_success = total tokens across all runs / |{ t : task.completion(t) }|
```
...
`unit` is omitted; the value counts tokens.
